        ...
        renderTeams(teams, period, currentPeriod, size) {
            ...
            //$scope.$apply(function() {
                    ...
                    getYears = function(team) {
                        ...
                        function processPeriods(key) {
                            ...
                            if (team.hasOwnProperty(key)) {
                                Object.keys(team[key]).map(month => {
                                    //console.log(month)
                                    year = (new Date(month)).getFullYear();
                                    monthNumber = (new Date(month)).getMonth();
                                    this.years[year] = {
                                        alias: year + '',
                                        label: year + ''
                                    }
                ...
                for (teamId in teams.teams) {
                    ...
                    months.map(month => {
                        if (!team.hasOwnProperty('prs') || !team.prs.hasOwnProperty(month)) {
                            return;
                        }
                        Object.keys(team.prs[month]).map(prId => {
                            pr = team.prs[month][prId]
                            team['prsNumberTotal']++
                            totals.prs++;
